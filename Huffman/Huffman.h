...
public:
	...
	bool decompress(const std::string& compressedFileName, const std::string& decompressedFileName);
private:
	std::map<char, std::string> huffmanMapCompress;
	std::map<std::string, char> huffmanMapDecompress;
	bool determineFrequency(const std::string& text,std::vector<HuffCode>& huffmanCodes);
